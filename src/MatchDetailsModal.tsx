...
import { loadCareer, loadSquad, type CareerSummary, type PlayerPosition, type SquadPlayer } from './squadData'

type View = 'formation' | 'players' | 'squad' | 'career'
const groups: PlayerPosition[] = ['GK','DEF','MID','FWD']

const modalCopy = {
  es:{title:'Detalle del partido y alineaciones',matchday:'Fecha',formation:'Formación',players:'Jugadores',viewSquad:'Ver plantilla completa',backPlayers:'Volver a jugadores',backSquad:'Volver a la plantilla',fullSquad:'Plantilla completa',career:'Resumen de carrera antes del Mundial',style:'Estilo de juego',strengths:'Fortalezas',form:'Forma reciente',position:'Posición',club:'Club',age:'Edad',foot:'Pie dominante',debut:'Debut con la selección',preview:'Datos de plantilla mostrados para la vista previa de la interfaz',loading:'Cargando datos públicos…',unavailable:'Información pública no disponible. Se muestra una vista previa segura.',close:'Cerrar detalle del partido',lineupNotice:'Las alineaciones suelen anunciarse aproximadamente una hora antes del inicio.',source:'Resumen de fuente pública',gk:'Porteros',def:'Defensas',mid:'Centrocampistas',fwd:'Delanteros',all:'Todos',select:'Seleccionar jugador',play:'Juego dinámico y disciplinado, adaptado a su posición.',strong:'Experiencia internacional · técnica · trabajo en equipo',recent:'Actividad reciente previa al Mundial.'},
  en:{title:'Match detail & lineups',matchday:'Matchday',formation:'Formation',players:'Players',viewSquad:'View full squad',backPlayers:'Back to players',backSquad:'Back to full squad',fullSquad:'Full squad',career:'Career summary before the World Cup',style:'Style of play',strengths:'Strengths',form:'Recent form',position:'Position',club:'Club',age:'Age',foot:'Dominant foot',debut:'National team debut',preview:'Squad data shown for interface preview',loading:'Loading public data…',unavailable:'Public information is unavailable. Showing a safe preview.',close:'Close match details',lineupNotice:'Lineups are usually announced approximately one hour before kickoff.',source:'Public-source summary',gk:'Goalkeepers',def:'Defenders',mid:'Midfielders',fwd:'Forwards',all:'All',select:'Select player',play:'Dynamic, disciplined play adapted to the player’s position.',strong:'International experience · technique · teamwork',recent:'Recent activity before the World Cup.'},
} as const
...
function PositionGroups({ players, onPlayer, language, compact=false }: { players:SquadPlayer[]; onPlayer:(player:SquadPlayer)=>void; language:Language; compact?:boolean }) {
  ...
  return <div className={`player-groups ${compact?'compact':''}`}>{groups.map(group=>{
    ...
    return <section key={group}><h4>{({GK:t.gk,DEF:t.def,MID:t.mid,FWD:t.fwd})[group]}</h4><div className="player-grid">{items.map(player=><button className="player-tile" key={`${player.name}-${player.number}`} onClick={()=>onPlayer(player)}><b>{player.number}</b><Avatar player={player}/><span>{player.name}<small>{player.role}</small></span></button>)}</div></section>
  })}</div>
}

function Formation({ team, players, onSquad, onPlayer, language }: {team:string;players:SquadPlayer[];onSquad:()=>void;onPlayer:(p:SquadPlayer)=>void;language:Language}) {
  const t=modalCopy[language]
  const eleven=players.slice(0,11)
  return <div className="formation-layout"><section><h3><TeamFlag team={team}/>{teamNames[team][language]} <small>4-3-3</small></h3><div className="pitch">{eleven.map((player,index)=><button key={player.name} className={`pitch-player p${index+1}`} onClick={()=>onPlayer(player)} aria-label={`${t.select}: ${player.name}`}><Avatar player={player}/><span>{player.name.split(' ').at(-1)}</span></button>)}</div></section><aside className="lineup-list"><h3>{teamNames[team][language]} {language==='es'?'plantilla':'squad'}</h3>{eleven.map(player=><button key={player.name} onClick={()=>onPlayer(player)}><b>{player.number}</b><span>{player.name}</span><small>{player.role}</small></button>)}<button className="modal-link" onClick={onSquad}>{t.viewSquad} ›</button></aside></div>
}

function PlayerSummary({ player, language, onCareer }: {player:SquadPlayer;language:Language;onCareer:()=>void}) {
  const t=modalCopy[language]
  return <aside className="player-summary"><h3>{player.name}</h3><div className="portrait-row"><Avatar player={player} large/><strong>{player.number}<small>{player.role}</small></strong></div><dl><div><dt>{t.position}</dt><dd>{player.role}</dd></div><div><dt>{t.club}</dt><dd>{player.club}</dd></div><div><dt>{t.age}</dt><dd>{player.age ?? '—'}</dd></div><div><dt>{t.foot}</dt><dd>{player.foot ?? '—'}</dd></div></dl><button className="modal-link" onClick={onCareer}>{t.career} ›</button></aside>
}
...
export function MatchDetailsModal({ match, language, onClose }: {match:Match;language:Language;onClose:()=>void}) {
  ...
  useEffect(()=>{let active=true;setLoading(true);loadSquad(team).then(next=>{if(!active)return;setPlayers(next);setSelected(current=>current&&next.find(item=>item.name===current.name)||next[0]||null);setLoading(false)});return()=>{active=false}},[team])
